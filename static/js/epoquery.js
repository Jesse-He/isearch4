		...
		$
				.each(
						...
						function(entryIndex, entry) {
							var content = entry.title + "*&&*" + entry.summerize + "*&&*" + entry.originalSource
							...
							if (entry.title && entry.title.length > 25)
								entry.title = titleFormat(entry.title, 25); // 缩减过长的标题
							if (entry.same) {
								rowhtml1 += "<div><ul class='page-header' style=\"list-style-type:none\" id="
										+ entry.id + ">" + "<li><a class='titleEpo' target='_blank' href='" + entry.url
										+ "'><i class='icon-tag'></i>" + entry.title + "</a>"
										+ "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[<strong>" + entry.originalSource
								...
								if (role == 1) {
									rowhtml1 += "<a id='"
											+ entry.id
											+ "' title='删除舆情记录' class=''style='cursor:pointer;' onclick='javascript:deleteWebPage(this);'>删除</a>&nbsp;&nbsp;|";
								}
								rowhtml1 += "&nbsp;&nbsp;"
										+ "<a id='collect"
										+ entry.id
										+ "' title='收藏' onclick='javascript:collect(this,1);' style='cursor:pointer;' value='"
										+ content
										+ "' class=''>收藏</a>&nbsp;&nbsp;<i class='icon-bookmark'></i></li></div></ul></div>";
							} else {
								rowhtml2 += "<div><ul class='page-header' style=\"list-style-type:none\" id="
										+ entry.id + ">" + "<li><a class='titleEpo' target='_blank' href='" + entry.url
										+ "'><i class='icon-tag'></i>" + entry.title + "</a>"
										+ "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[<strong>" + entry.originalSource
										+ " </strong>" + entry.pubTime
								...
								rowhtml2 += "&nbsp;&nbsp;" + "<a id='collect" + entry.id
										+ "' title='收藏' onclick='javascript:collect(this,1);' value='" + content
										+ "' style='cursor:pointer;' class=''>收藏</a></div></li></ul></div>";
							}
							startIndex = startIndex + 1;
						});
		htmlContent = rowhtml1 + rowhtml2;
		$("#dataArea").append(htmlContent);
	}
	searchCollect(1);// 查询收藏
}
// 删除
function deleteWebPage(obj) {
	$.ajaxSettings.async = false;
	var ids = obj.id;
...
function commonpartDisplay() {
	...
	// 加载搜索引擎
	var content2 = "";
	var url1 = "../../handler/metasearch/queryHotMetaSearch";
	$.post(url1, {
		"topNum" : 5
	}, function(data) {
		var resultData = data.data.data;
		if (resultData != null) {
			$.each(resultData, function(entryIndex, entry) {
				content2 = content2
						+ "<a href=\"javascript:void(0);\" class='rio' style='cursor:pointer;' name=\"i4\" id=\"i4"
						+ entry.id + "\" onclick=\"radioDisplay(this);\">" + entry.indicator + "</a>";
			});
		}
	});
	var radioHtml = "<div class=\"tabbable well\"> <ul class=\"nav nav-tabs\">"
			+ "<li class='active'><a href=\"#web-filter\" data-toggle=\"tab\"><i class='icon-globe'></i>网页舆情</a></li>"
			+ "<li><a style='cursor:pointer;' data-toggle=\"tab\" onclick='turnOther();'><i class=' icon-tasks'></i>微博舆情</a></li></ul>"
			//
			...
			+ "<a href=\"javascript:void(0);\" style='cursor:pointer;' class='rio reactive' name=\"i2\" id=\"i21\" onclick=\"javascript:radioDisplay(this);\">不限</a>"
			+ "<a href=\"javascript:void(0);\" style='cursor:pointer;' class='rio' name=\"i2\"  id=\"i22\" onclick=\"javascript:radioDisplay(this);\">一般关注</a>"
			+ "<a href=\"javascript:void(0);\" style='cursor:pointer;' class='rio' name=\"i2\" id=\"i23\" onclick=\"javascript:radioDisplay(this);\">比较关注</a>"
			+ "<a href=\"javascript:void(0);\" style='cursor:pointer;' class='rio' name=\"i2\" id=\"i24\"  onclick=\"javascript:radioDisplay(this);\">重点关注</a>"
			+ "<a href=\"javascript:void(0);\" style='cursor:pointer;' class='rio' name=\"i2\" id=\"i25\"  onclick=\"javascript:radioDisplay(this);\">发生预警</a></div>"
			//
			+ "<div><span class='label label-info'>舆情主题</span>"
			+ "<a href=\"javascript:void(0);\" class='rio reactive' style='cursor:pointer;' name=\"i3\" id=\"i30\" onclick=\"javascript:radioDisplay(this);\">不限</a>"
			+ content1
			+ "</div>"
			//
			+ "<div><span class='label label-info'>媒体类型</span>"
			+ "<a href=\"javascript:void(0);\" class='rio reactive' style='cursor:pointer;' name=\"i4\" id=\"i40\" onclick=\"javascript:radioDisplay(this);\">不限</a>"
			+ content2
			+ "</div>"
			//
			+ "<div><span class='label label-info'>舆情时间</span>"
			+ "<a href=\"javascript:void(0);\" class='rio reactive' style='cursor:pointer;' name=\"i5\" id=\"i51\" onclick=\"javascript:radioDisplay(this);\">不限</a>"
			+ "<a href=\"javascript:void(0);\" class='rio' style='cursor:pointer;' name=\"i5\" id=\"i52\" onclick=\"javascript:radioDisplay(this);\">今日最新</a>"
			+ "<a href=\"javascript:void(0);\" class='rio' style='cursor:pointer;' name=\"i5\" id=\"i53\" onclick=\"javascript:radioDisplay(this);\">最近两天</a>"
			+ "<a href=\"javascript:void(0);\" class='rio' style='cursor:pointer;' name=\"i5\" id=\"i54\"  onclick=\"javascript:radioDisplay(this);\">最近一周</a>"
			+ "<a href=\"javascript:void(0);\" class='rio' style='cursor:pointer;' name=\"i5\" id=\"i55\"  onclick=\"javascript:radioDisplay(this);\">最近两周</a>"
			+ "<a href=\"javascript:void(0);\" class='rio' style='cursor:pointer;' name=\"i5\" id=\"i56\"  onclick=\"javascript:radioDisplay(this);\">最近一月</a>"
			+ "<a href=\"javascript:void(0);\" class='rio' style='cursor:pointer;' name=\"i5\" id=\"i57\"  onclick=\"javascript:radioDisplay(this);\">自选时段</a>"
			...
			// "<div class='span9' style='height:200px;' id='trends'></div>"

			+ "</div><div class=\"tab-pane\" id=\"web-trends\"></div></div></div>";
	// 最后一行切换
...
function sentByTimeDisplay() {
	...
	var stepNum = 1;
	var url = '../../handler/epostat/sentByTimeRange';
	$.post(url, params, function(data) {
		if (data.data != null && data.data.statData != null && data.data.statData.length > 0) {
			var resultData = data.data.statData;
			timeUnit = data.data.timeUnit;
...
function getParams() {
	...
	// 判断当前的搜索引擎哪个被选中了
	if (p4 == "i40") {
		params.metaSearch = null;
	} else {
		metaSearch = parseInt(p4.replace("i4", ""));
		params.metaSearch = metaSearch;
	}
	// 扫描当前舆情事件选中按钮在哪里
	if (p5 == "i52") {
